...
/*global BaseApi BangumiApi EmbyApi TraktApi TmdbApi DoubanApi*/
...
class EmbyApi extends BaseApi {
    ...
    async getItems({ genre = '', types = 'Movie,Series,Video', fields = null, startIndex = 0,
        ...
        let userPath = (this.userId) ? `Users/${this.userId}/` : ''; // 加用户ID会将不同路径的相同条目合并为一个。但会慢一点。

        return await this._get(`${userPath}Items`, params);
    }
    ...
    async searchByProviiderIds(tkIds, type = undefined) {
        ...
        const idsParam = Object.entries(tkIds)
            .filter(([k, v]) => v && (type || k.toLowerCase() !== 'tmdb')) // 修改这一行
            .map(([k, v]) => `${k}.${v}`)
            .join(',');
...
class DoubanApi extends BaseApi {
    constructor() {
        super('https://api.douban.com/v2'); // /v2 会被清除
        this.headers = {
            'Content-Type': 'application/x-www-form-urlencoded; charset=utf8',
        };
    }

    async getIdByApi(imdbId) {
        let data = await this._req(
            'POST',
            `v2/movie/imdb/${imdbId}`,
            null,
            null,
    ...
    async getIdByWikidata(imdbId) {
        let wikidataApi = new BaseApi('https://query.wikidata.org');
        let wikidataUrl = 'sparql?format=json&query=' +
            encodeURIComponent(`SELECT * WHERE { ?s wdt:P345 "${imdbId}". OPTIONAL { ?s wdt:P4529 ?Douban_film_ID. } }`);
        let wikidataRes = await wikidataApi._get(wikidataUrl);
    ...
    async getDoubanId(imdbId) {
        let doubanId = await this.getIdByApi(imdbId);
        if (doubanId) { return doubanId; }
        doubanId = await this.getIdByWikidata(imdbId);
        return doubanId;
    }

    async getDoubanIdWithStorage(dbClass, imdbId) {
        let imdbDoubanDb = new dbClass('imdb|douban');
        let doubanId = imdbDoubanDb.get(imdbId);
        if (doubanId == '_') { return; }
        if (doubanId) { return doubanId; }

        doubanId = await this.getDoubanId(imdbId);
        if (doubanId) {
